...
export const AlertsView: React.FC<Props> = ({ currentTrends, onSelectTrend, onShowToast }) => {
  ...
  const handleAddAlert = (e: React.FormEvent) => {
    ...
    const exists = alerts.some(a => a.keyword.toLowerCase() === keyword.toLowerCase() && a.region === (currentTrends[0]?.region || 'ID'));
    if (exists) {
    ...
    const newAlert: TrackedAlert = {
      ...
      keyword,
      region: currentTrends[0]?.region || 'ID',
      targetRank: 10,
      enabled: true,
  ...
  return (
    <div id="alerts-view" className="space-y-6 animate-in fade-in duration-300">
      ...
      <form onSubmit={handleAddAlert} className="flex gap-2 p-4 bg-slate-800/40 border border-white/10 backdrop-blur-md rounded-2xl shadow-xl">
        <input type="text" placeholder="Enter keyword to monitor..." value={newKeyword} onChange={e => setNewKeyword(e.target.value)} className="flex-1 px-4 py-2 bg-slate-900/50 border border-white/10 rounded-xl text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 placeholder:text-slate-500" />
        <button type="submit" className="px-4 py-2 rounded-xl bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-500 transition-all flex items-center gap-1.5 shrink-0 shadow-lg shadow-indigo-600/25">
          ...
          return (
            <div key={alert.id} className={`p-4 rounded-2xl border backdrop-blur-md transition-all flex items-center justify-between gap-4 shadow-xl ${isTrendingNow ? 'bg-amber-500/15 border-amber-500/30' : 'bg-slate-800/40 border-white/10'}`}>
              <div className="flex items-center gap-3">
                <div className={`p-2.5 rounded-xl ${isTrendingNow ? 'bg-amber-500 text-white animate-bounce' : 'bg-slate-800/60 text-slate-400 border border-white/10'}`}><Bell className="w-4 h-4" /></div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="font-bold text-sm text-white">{alert.keyword}</h4>
                    {isTrendingNow && <span className="text-[10px] font-mono font-bold tracking-wider px-2 py-0.5 rounded-lg bg-amber-500 text-white">TRENDING: #{matchingTrend?.rank}</span>}
                  </div>
                  <p className="text-xs text-slate-400 mt-0.5">Region: {alert.region} • Target: Top {alert.targetRank} • Created: {new Date(alert.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {matchingTrend && <button onClick={() => onSelectTrend(matchingTrend)} className="px-3.5 py-1.5 rounded-xl bg-indigo-600 text-white text-xs font-semibold hover:bg-indigo-500 transition-colors">Analyze</button>}
                <button onClick={() => handleDeleteAlert(alert.id)} className="p-2 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-colors" title="Remove alert"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          );
        })}
